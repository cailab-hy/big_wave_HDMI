...
def main(cfg):
    ...
    visualize_camera = cfg.get("visualize_camera", True)
    camera_mode = cfg.get("camera_mode", "ego_rgb")  # "ego_rgb" or "ego_depth"
    camera_viewer = cfg.get("camera_viewer", "isaac")  # "isaac" (IsaacLab UI window) or "opencv"
    ui_window = None
    if visualize_camera and camera_viewer == "isaac":
        from active_adaptation.utils.ui_image import ImageWindow
        try:
            ui_window = ImageWindow("Robot Camera", width=640, height=480)
        except Exception as e:
            print(f"[WARN] Could not create the IsaacLab camera window ({e}), falling back to OpenCV.")
            camera_viewer = "opencv"
    if visualize_camera and camera_viewer == "opencv":
        import cv2
    ...
    # Print camera pose info
    if visualize_camera and "tiled_camera" in env.base_env.scene.sensors:
        cam = env.base_env.scene.sensors["tiled_camera"]
        print("\n=== Camera Info ===")
        print(f"Prim path: {cam.cfg.prim_path}")
        print(f"Offset pos: {cam.cfg.offset.pos}")
        print(f"Offset rot: {cam.cfg.offset.rot}")
        print(f"Convention: {cam.cfg.offset.convention}")
        print(f"Resolution: {cam.cfg.width}x{cam.cfg.height}")
        cam_pos_w = cam.data.pos_w[0].cpu().numpy()
        cam_quat_w = cam.data.quat_w_ros[0].cpu().numpy()
        print(f"World position: {cam_pos_w}")
        print(f"World quaternion (ROS): {cam_quat_w}")
        print("===================\n")
    ...
    with torch.inference_mode(), set_exploration_type(ExplorationType.MODE):
        ...
        for i in itertools.count():
            ...
            if visualize_camera:
                frame = env.base_env.render(mode=camera_mode)
                if frame is not None:
                    frame = frame.astype(np.uint8)
                    if ui_window is not None:
                        # the UI expects RGB; ego_depth comes back as a BGR colormap
                        if camera_mode != "ego_rgb":
                            frame = frame[:, :, ::-1]
                        ui_window.update(frame)
                    else:
                        if camera_mode == "ego_rgb":
