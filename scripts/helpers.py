...
def make_env_policy(cfg: DictConfig):
    ...
    if cfg.get("resume_training", False):
        for name, optimizer_state in state_dict.get("optimizers", {}).items():
            optimizer = getattr(policy, name, None)
            if isinstance(optimizer, torch.optim.Optimizer):
                optimizer.load_state_dict(optimizer_state)
                print(colored(f"[Info]: Load optimizer '{name}' from checkpoint.", "green"))

    policy._loaded_training_state = state_dict.get("training_state", {})
