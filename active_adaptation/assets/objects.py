...
class CustomArticulation(Articulation):
    def _create_buffers(self):
        ...
        assert (
            len(self.joint_names) == 1
        ), "DoorArticulation should have exactly one joint."
    ...
    def write_data_to_sim(self):
        ...
        self.set_joint_effort_target(
            self.custom_torques.unsqueeze(-1), joint_ids=[self.custom_joint_id]
        )
...
BLOCK_CFG = RigidObjectCfg(
    ...
    spawn=sim_utils.UsdFileCfg(
        usd_path=f"{ASSET_PATH}/objects/scenario/Block.usd",
        activate_contact_sensors=True,
...
OPENBOX_CFG = RigidObjectCfg(
    prim_path="{ENV_REGEX_NS}/openbox",
    spawn=sim_utils.UsdFileCfg(
        usd_path=f"{ASSET_PATH}/objects/scenario/OpenBox.usd",
        activate_contact_sensors=True,
        rigid_props=sim_utils.RigidBodyPropertiesCfg(
            rigid_body_enabled=True,
        ),
