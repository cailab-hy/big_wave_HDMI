...
class root_ang_vel_history(Observation):
    def __init__(self, env, noise_std: float=0., history_steps: list[int]=[1], scale: float=0.25):
        super().__init__(env)
    ...
    def compute(self) -> torch.Tensor:
        return self.buffer[:, self.history_steps].reshape(self.num_envs, -1)  * self.scale
...
class joint_vel_history(Observation):
    ...
    def reset(self, env_ids):
        ...
        joint_vel2 = self.asset.data.joint_vel[env_ids][:, self.joint_ids]
        assert torch.allclose(joint_vel, joint_vel2)
        self.buffer[env_ids] = self.asset.data.joint_vel[env_ids.unsqueeze(1), self.joint_ids.unsqueeze(0)].unsqueeze(1)
    ...
    def compute(self):
        ...
        joint_vel = joint_vel * self.joint_mask
        joint_vel_selected = joint_vel[:, self.history_steps]
        return joint_vel_selected.reshape(self.num_envs, -1) * self.scale
