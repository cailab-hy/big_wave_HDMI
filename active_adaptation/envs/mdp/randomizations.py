...
class push_by_vel(Randomization):
    def __init__(self, env, velocity_range={"x": (-1.0, 1.0), "y": (-1.0, 1.0)}, interval=100):
        super().__init__(env)
        self.asset: Articulation = self.env.scene["robot"]
    ...
    def step(self, substep):
        ...
        if self.steps % self.interval == 0:
            ...
            vel_w = self.asset.data.root_vel_w.clone()
            vel_w += math_utils.sample_uniform(ranges[:, 0], ranges[:, 1], vel_w.shape, device=self.asset.device)
            # set the velocities into the physics simulation
            self.asset.write_root_velocity_to_sim(vel_w)
