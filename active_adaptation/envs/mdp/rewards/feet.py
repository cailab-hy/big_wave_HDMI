...
class feet_slip_modified(feet_slip):
    def __init__(
        self, env, body_names: str, weight: float, tolerance: float = 0.0, enabled: bool = True
    ):
        super().__init__(env,body_names,weight,tolerance, enabled)

    def compute(self) -> torch.Tensor:
        contacts = self.contact_sensor.data.net_forces_w_history[:, :, self.body_ids, :].norm(dim=-1).max(dim=1)[0] > 1.0
        body_vel = self.asset.data.body_lin_vel_w[:, self.articulation_body_ids, :2]
