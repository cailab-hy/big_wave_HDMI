...
class RobotTracking(Command):
    def __init__(
        ...
        init_joint_noise_exclude_names: List[str] | str | None = None,
        ...
        with torch.device(self.device):
            ...
            # Per-joint mask for reset-time joint noise.
            # 1.0 => apply noise, 0.0 => no noise for that joint.
            self.init_joint_noise_mask = torch.ones(len(asset_joint_names), device=self.device)
            if init_joint_noise_exclude_names is not None:
                exclude_joint_ids, _ = self.asset.find_joints(init_joint_noise_exclude_names)
                self.init_joint_noise_mask[exclude_joint_ids] = 0.0
    ...
    def sample_init(self, env_ids: torch.Tensor) -> None:
        ...
        joint_pos_noise *= self.init_joint_noise_mask.unsqueeze(0)
        joint_vel_noise *= self.init_joint_noise_mask.unsqueeze(0)
...
class RobotObjectTracking(RobotTracking):
    def __init__(
        ...
        # for contact rewards
        contact_eef_body_name: List[str] = ["left_hand_link", "right_hand_link"],                  # G1: ["left_wrist_yaw_link", "right_wrist_yaw_link"],
        contact_frc_eef_body_name: List[str | List[str]] = ["left_hand_link", "right_hand_link"],  # G1: ["left_wrist_(roll|pitch|yaw)_link", "right_wrist_(roll|pitch|yaw)_link"],
        ...
        **kwargs
        ...
        # set very high inertia for extra objects to prevent rotation from collisions
        for extra_obj in self.extra_objects:
            inertias = extra_obj.root_physx_view.get_inertias().clone()
            # shape: (num_instances, 9) — row-major 3x3: [Ixx, Ixy, Ixz, Iyx, Iyy, Iyz, Izx, Izy, Izz]
            inertias[:] = 0.0
            inertias[:, 0] = 1e6  # Ixx
            inertias[:, 4] = 1e6  # Iyy
            inertias[:, 8] = 1e6  # Izz
            extra_obj.root_physx_view.set_inertias(inertias, torch.arange(extra_obj.num_instances))
