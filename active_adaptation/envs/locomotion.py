    ...
    def __init__(self, cfg):
        ...
        if self.backend == "isaac" and self.sim.has_gui():
            ...
            self.viewport_camera_controller = ViewportCameraController(
                self,
                ViewerCfg(
                    self.cfg.viewer.eye, self.cfg.viewer.lookat, origin_type="env"
                ),
            )

            look_at_env_id = self.lookat_env_i
            self.sim.set_camera_view(
                eye=self.scene.env_origins[look_at_env_id].cpu()
                + torch.as_tensor(self.cfg.viewer.eye),
                target=self.scene.env_origins[look_at_env_id].cpu()
                + torch.as_tensor(self.cfg.viewer.lookat),
            )
        ...
        if active_adaptation.get_backend() == "isaac":
            ...
            if hasattr(self.cfg.robot, "override_params"):
                ...
                update_class_from_dict(
                    scene_cfg.robot, self.cfg.robot.override_params, _ns=""
                )

            scene_cfg.robot.prim_path = "{ENV_REGEX_NS}/Robot"
            robot_type = self.cfg.robot.get("robot_type", self.cfg.robot.name)
            # Handle both UsdFileCfg (usd_path) and UrdfFileCfg (asset_path)
            if (
                hasattr(scene_cfg.robot.spawn, "usd_path")
                and scene_cfg.robot.spawn.usd_path
            ):
                scene_cfg.robot.spawn.usd_path = scene_cfg.robot.spawn.usd_path.format(
                    ROBOT_TYPE=robot_type
                )
            elif (
                hasattr(scene_cfg.robot.spawn, "asset_path")
                and scene_cfg.robot.spawn.asset_path
            ):
                scene_cfg.robot.spawn.asset_path = (
                    scene_cfg.robot.spawn.asset_path.format(ROBOT_TYPE=robot_type)
                )
            ...
            if "object_asset_name" in self.cfg.command:
                ...
                obj_cfg.spawn.usd_path = obj_cfg.spawn.usd_path.format(
                    OBJECT_TYPE=obj_type
                )
                print(
                    f"Using object type {obj_type} with asset {obj_cfg.spawn.usd_path}"
                )
                ...
                contact_geom_prim_path = (
                    "{ENV_REGEX_NS}/" + obj_name + "/" + obj_contact_body_name
                )
            ...
            if body_scale_rand is not None:
                ...
                asset.spawn.homogeneous_scale = body_scale_rand.get(
                    "homogeneous_scale", False
                )
                print(
                    f"Randomized {body_scale_rand.name} scale to {asset.spawn.scale_range}"
                )

            scene_cfg.terrain = TERRAINS[self.cfg.terrain]
            scene_cfg.contact_forces = ContactSensorCfg(
                prim_path="{ENV_REGEX_NS}/Robot/.*(ankle_roll_.*|wrist_.*)_link",
                history_length=3,
                track_air_time=True,
            )
            ...
            if self.cfg.get("enable_cameras", False):
                ...
                tiled_camera: TiledCameraCfg = TiledCameraCfg(
                    prim_path="/World/envs/env_.*/Robot/camera_head_link/front_cam",
            ...
            sim_cfg.physx.gpu_max_rigid_patch_count = 2**19  # 524288
            ...
            self.sim.set_camera_view(
                eye=self.cfg.viewer.eye, target=self.cfg.viewer.lookat
            )
            try:
            ...
            path = os.path.join(os.getcwd(), "asset_meta_TienkungPro.json")
            # path = os.path.join(os.getcwd(), "asset_meta_G1.json")
