...
def nearest_time(ts_target, ts_source, x):
    idx = np.clip(np.round(
        (ts_target - ts_source[0]) / (ts_source[1] - ts_source[0])
    ).astype(int), 0, ts_source.shape[0] - 1)
    return x[idx]


def interpolate(motion, source_fps: int, target_fps: int):
    if source_fps != target_fps:
        in_keys = ["body_pos_w", "body_lin_vel_w", "body_quat_w", "body_ang_vel_w", "joint_pos", "joint_vel", "object_contact"]
        extra_keys = set(motion.keys()) - set(in_keys) - ["body_names", "fps"]
        if extra_keys:
        ...
        motion["object_contact"] = nearest_time(ts_target, ts_source, motion["object_contact"])
    return motion
